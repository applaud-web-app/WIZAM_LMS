...
class ManageTest extends Controller
{
    ...
    public function quizzesSchedules(Request $request,$id){
        ...
        if($quiz){
            if ($request->ajax()) {
                $sections = QuizSchedule::where('quizzes_id',$id)->whereIn('status',[0,1,3]);
    ...
    public function saveQuizzesSchedules(Request $request,$id){
        ...
        // Fetch the schedule by its ID
        $schedule = QuizSchedule::where('id',$scheduleId)->first();
            ...
            if ($request->ajax()) {
                ...
                return DataTables::of($sections)
                    ...
                    ->addColumn('action', function ($section) {
                        ...
                        $deleteUrl = route('delete-exam-result',[$section->uuid]); // Update route name
                        return '
