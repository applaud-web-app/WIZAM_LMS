...
<section class="mx-[30px] min-h-[calc(100vh-195px)] mb-[30px] ssm:mt-[30px] mt-[15px]">
            ...
            <div class="mb-[30px]">
                <div class="bg-white dark:bg-gray-800 rounded-lg p-5">
                    <div class="flex items-center justify-between">
                        <!-- Step 1 -->
                        <a href="{{route('quizzes-detail',['id'=>$quizz->id])}}" class="flex-1 text-center">
                        ...
                        <!-- Step 2 -->
                        <a href="{{route('quizzes-setting',['id'=>$quizz->id])}}" class="flex-1 text-center">
                            <div class="relative flex flex-col items-center">
                                <div class="w-8 h-8 rounded-full bg-gray-300 text-gray-500 flex items-center justify-center">
                                    2
                                </div>
                        ...
                        <!-- Step 3 -->
                        <a href="{{route('quizzes-question',['id'=>$quizz->id])}}" class="flex-1 text-center">
                            <div class="relative flex flex-col items-center">
                        ...
                        <div class="w-[40px] h-[2px] bg-gray-300"></div>
                        <!-- Step 2 -->
                        <a href="{{route('quizzes-schedules',['id'=>$quizz->id])}}" class="flex-1 text-center">
                            <div class="relative flex flex-col items-center">
                                <div class="w-8 h-8 rounded-full bg-gray-300 text-gray-500 flex items-center justify-center">
                                    4
                                </div>
                                <div class="text-gray-400 mt-2">Schedules</div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg p-5 mb-[30px]">
        <form class="grid grid-cols-12 gap-5" id="filterForm">
            ...
            <input type="hidden" name="quiz_id" value="{{$quizz->id}}" id="quizId">
    ...
    <!-- Question Card List -->
    <form action="{{route('update-quizzes-question',['id'=>$quizz->id])}}" method="POST" autocomplete="off" class="bg-white dark:bg-box-dark m-0 p-[20px] text-body dark:text-subtitle-dark text-[15px] rounded-10 relative h-full">
        @csrf
        <div class="">
            <h3>Currently Viewing Questions</h3>
            <div class="flex mt-2"><button type="button" data-type="all" data-id="{{$quizz->id}}" class="fetch_question text-gray-500">View Questions</button> <span class="mx-3">|</span> <button type="button" data-type="new" data-id="{{$quizz->id}}" class="fetch_question text-gray-500">Add Questions</button></div>
        </div>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            $('#filterForm').on('submit', function(event) {
                ...
                $.ajax({
                    url: "{{route('filter-quizzes-question')}}", // Your route
                    method: 'POST',
                    data: formData,
    ...
    $(document).ready(function() {
        $('.fetch_question').on('click', function() {
            ...
            // Make AJAX request
            $.ajax({
                url: "{{ route('fetch-quizzes-question') }}",
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
    ...
    <script>
        function removeQuestion($question){
            ...
            $.ajax({
                url: "{{route('remove-quizzes-question')}}", // Adjust this URL to your actual endpoint
                type: 'POST',
                data: {
