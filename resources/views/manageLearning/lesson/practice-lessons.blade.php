...
<section class="mx-[30px] min-h-[calc(100vh-195px)] mb-[30px] ssm:mt-[30px] mt-[15px]">
            ...
            <div class="mb-[30px]">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg p-5">
                    <div class="flex items-center justify-between">
                        <!-- Step 1 -->
                        <a href="{{route('configure-lessons')}}" class="flex-1 text-center">
                        ...
                        <a href="{{url()->full()}}" class="flex-1 text-center">
                            <div class="relative flex flex-col items-center">
                                ...
                                <div class="text-primary mt-2">Add/Remove Lesson</div>
                            </div>
                        </a>
                    </div>
                </div>
        ...
        <form class="grid grid-cols-12 gap-5" id="filterForm">
            ...
            <div class="col-span-12 sm:col-span-6 md:col-span-6">
                ...
                <div class="flex flex-col">
                    ...
                    <div class="mb-[0.125rem] block min-h-[1.5rem]">
                        <input class="relative ltr:float-left rtl:float-right me-[6px] mt-[0.15rem] h-[1.125rem] w-[1.125rem] appearance-none rounded-[0.25rem] border-1 border-solid border-normal outline-none before:pointer-events-none before:absolute before:h-[10px] before:w-[0.5px] before:scale-0 before:rounded-full before:bg-transparent before:opacity-0 before:content-[''] checked:border-primary checked:bg-primary checked:before:opacity-[0.16] checked:after:absolute checked:after:mt-0 checked:after:ms-[5px] checked:after:block checked:after:h-[10px] checked:after:w-[5px] checked:after:rotate-45 checked:after:border-[0.125rem] checked:after:border-l-0 checked:after:border-t-0 checked:after:border-solid checked:after:border-white checked:after:bg-transparent checked:after:content-[''] hover:cursor-pointer hover:before:opacity-[0.04] dark:border-white/10 dark:checked:border-primary dark:checked:bg-primary after:top-[2px]" type="checkbox" name="difficultyLevel[]" value="veryHard" id="difficultyLevel5" autocompleted="">
    ...
    <!-- Question Card List -->
    <form action="{{route('update-practice-lessons',['category'=>$subcategory->id,'skill'=>$skill->id])}}" method="POST" autocomplete="off" class="bg-white dark:bg-box-dark m-0 p-[20px] text-body dark:text-subtitle-dark text-[15px] rounded-10 relative h-full">
        @csrf
        <div class="">
            <h3>Currently Viewing Lessons</h3>
            <div class="flex mt-2"><button type="button" data-type="all" data-category="{{$subcategory->id}}" data-skill="{{$skill->id}}" class="fetch_question text-gray-500">View Lessons</button> <span class="mx-3">|</span> <button type="button" data-type="new" data-category="{{$subcategory->id}}" data-skill="{{$skill->id}}" class="fetch_question text-gray-500">Add Lessons</button></div>
        </div>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            $('#filterForm').on('submit', function(event) {
                ...
                $.ajax({
                    url: "{{route('filter-practice-lesson')}}", // Your route
                    method: 'POST',
                    data: formData,
                    success: function(response) {
                        ...
                        response.lessons.forEach(function(lesson) {
                            ...
                            let topicDisplay = '';
                            if (lesson.skill && lesson.skill.name) {
                                skillDisplay = lesson.skill.name;
                            }
                            
                            content= `<div class="border border-gray-200 dark:border-gray-700 rounded-lg p-5 mb-4">
                                        <div class="flex justify-between">
                                            <div class="flex items-center">
                                                <input class="relative ltr:float-left rtl:float-right me-[6px] mt-[0.15rem] h-[1.125rem] w-[1.125rem] appearance-none rounded-[0.25rem] border-1 border-solid border-normal outline-none before:pointer-events-none before:absolute before:h-[10px] before:w-[0.5px] before:scale-0 before:rounded-full before:bg-transparent before:opacity-0 before:content-[''] checked:border-primary checked:bg-primary checked:before:opacity-[0.16] checked:after:absolute checked:after:mt-0 checked:after:ms-[5px] checked:after:block checked:after:h-[10px] checked:after:w-[5px] checked:after:rotate-45 checked:after:border-[0.125rem] checked:after:border-l-0 checked:after:border-t-0 checked:after:border-solid checked:after:border-white checked:after:bg-transparent checked:after:content-[''] hover:cursor-pointer hover:before:opacity-[0.04] dark:border-white/10 dark:checked:border-primary dark:checked:bg-primary after:top-[2px]" type="checkbox" name="lesson[]" value="${lesson.id}" autocompleted="">
                                        ...
                                        <h4 class="mt-2">${lesson.title}</h4>
                                        <div class="text-body dark:text-subtitle-dark mt-2 flex">
                                        ${lesson.description}
                                        </div>
                                        <div class="space-y-3 mt-4">
                                            <div>
                                                <strong>Read Time:</strong> ${lesson.read_time}
                                            </div>
                                            <div>
                                                <strong>Difficulty Level:</strong> ${lesson.level}
                                            </div>
                                        </div>
                                    </div>`;

                            $('#lessonsContainer').append(content);
    ...
    $(document).ready(function() {
        $('.fetch_question').on('click', function() {
            ...
            // Make AJAX request
            $.ajax({
                url: "{{ route('fetch-practice-lessons') }}", // Ensure the route is correctly defined in Laravel
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                ...
                success: function(response) {
                        ...
                        response.lessons.forEach(function(lesson) {
                            ...
                            let checkBox = ``;
                            if(actionType == "all"){
                                remvebtn = `<button type="button" class="bg-danger hover:bg-primary-hbr border-solid border-1 border-danger text-white dark:text-title-dark text-[14px] leading-[22px] inline-flex items-center justify-center rounded-[4px] py-1 px-3  transition duration-300 ease-in-out" onclick="removeQuestion('${lesson.id}')">Remove</button>`; 
                            }else{
                                checkBox = `<input class="relative ltr:float-left rtl:float-right me-[6px] mt-[0.15rem] h-[1.125rem] w-[1.125rem] appearance-none rounded-[0.25rem] border-1 border-solid border-normal outline-none before:pointer-events-none before:absolute before:h-[10px] before:w-[0.5px] before:scale-0 before:rounded-full before:bg-transparent before:opacity-0 before:content-[''] checked:border-primary checked:bg-primary checked:before:opacity-[0.16] checked:after:absolute checked:after:mt-0 checked:after:ms-[5px] checked:after:block checked:after:h-[10px] checked:after:w-[5px] checked:after:rotate-45 checked:after:border-[0.125rem] checked:after:border-l-0 checked:after:border-t-0 checked:after:border-solid checked:after:border-white checked:after:bg-transparent checked:after:content-[''] hover:cursor-pointer hover:before:opacity-[0.04] dark:border-white/10 dark:checked:border-primary dark:checked:bg-primary after:top-[2px]" type="checkbox" name="lesson[]" value="${lesson.id}" autocompleted="">`;
                            }
                            
                            content= `<div class="border border-gray-200 dark:border-gray-700 rounded-lg p-5 mb-4" id="lesson-${lesson.id}">
                                        <div class="flex justify-between">
                                            <div class="flex items-center">
                                                ${checkBox}
                                                <h4 class="ml-1 bg-primary/10 text-primary text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">${skillDisplay}</h4>
    ...
    <script>
        function removeQuestion($question){
            ...
            $.ajax({
                url: "{{route('remove-practice-lessons')}}", // Adjust this URL to your actual endpoint
                type: 'POST',
                data: {
                ...
                error: function(xhr) {
                    ...
                    iziToast.error({
                        title: 'Error',
                        message: 'Failed to remove the lesson. Please try again.',
                        position: 'topRight'
                    });
                }
            });
