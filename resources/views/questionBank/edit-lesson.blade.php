...
<section class="mx-[30px] min-h-[calc(100vh-195px)] mb-[30px] ssm:mt-[30px] mt-[15px]">
   ...
   <div class="grid grid-cols-12 gap-5">
      <div class="col-span-12">
         ...
         <div class="leading-[1.8571428571] flex flex-wrap sm:justify-between justify-center items-center ssm:mb-[24px] mb-[18px] max-sm:flex-col gap-x-[15px] gap-y-[5px]">
            <!-- Title -->
            <h4 class="capitalize text-[20px] text-dark dark:text-title-dark font-semibold">Edit Lesson</h4>
   ...
   <div class="bg-white dark:bg-box-dark m-0 p-0 text-body dark:text-subtitle-dark text-[15px] rounded-10 relative h-full">
      ...
      <!-- Blog Form -->
      <div class="p-[25px]">
         <form id="addLessonForm" action="{{url()->full()}}" method="POST" enctype="multipart/form-data" class="grid grid-cols-12 gap-[25px]">
            @csrf
            <!-- lesson Title -->
            <div class="col-span-12">
               ...
               <input type="text" name="lesson_title" value="@isset($lesson->title){{$lesson->title}}@endisset" id="lesson_title" class="w-full px-4 py-3 rounded-4 border-normal border-1 dark:bg-box-dark-up dark:border-box-dark-up text-body dark:text-subtitle-dark" placeholder="Enter lesson title" required>
            </div>
            ...
            <div class="col-span-12">
               ...
               <textarea name="description" id="description" rows="4" required class="summernote w-full px-4 py-3 rounded-4 border-normal border-1 dark:bg-box-dark-up dark:border-box-dark-up text-body dark:text-subtitle-dark" placeholder="Enter a short description">@isset($lesson->description){{$lesson->description}}@endisset</textarea>
            </div>
                   ...
                   <option value="" disabled selected>Select Skill</option>
                    @isset($skill)
                        @foreach ($skill as $item)
                            <option value="{{$item->id}}" {{$lesson->skill_id == $item->id ? "selected" : ""}}>{{$item->name}}</option>
                        @endforeach
                    @endisset
               </select>
            </div>
                  ...
                  <option value="" disabled selected>Select Topic</option>
                    @isset($topic)
                        @foreach ($topic as $item)
                            <option value="{{$item->id}}" {{$lesson->topic_id == $item->id ? "selected" : ""}}>{{$item->name}}</option>
                        @endforeach
                    @endisset
               </select>
            </div>
            ...
            <div class="col-span-12">
               ...
               <input type="text" name="tags" id="tags" class="w-full px-4 py-3 rounded-4 border-normal border-1 dark:bg-box-dark-up dark:border-box-dark-up text-body dark:text-subtitle-dark" value="@isset($lesson->tags){{$lesson->tags}}@endisset" placeholder="tags" >
            </div>
            ...
            <div class="col-span-12 md:col-span-6">
               ...
               <select name="difficulty_level" id="difficulty_level" class="w-full px-4 py-3 rounded-4 border-normal border-1 dark:bg-box-dark-up dark:border-box-dark-up text-body dark:text-subtitle-dark" required>
                  ...
                  <option value="very_easy" {{$lesson->level == "very_easy" ? "selected" : ""}}>Very Easy</option>
                  ...
                  <option value="very_hard" {{$lesson->level == "very_hard" ? "selected" : ""}}>Very Hard</option>
               </select>
            </div>
            ...
            <div class="col-span-12 md:col-span-6">
               ...
               <input type="number" name="read_time" id="read_time" class="w-full px-4 py-3 rounded-4 border-normal border-1 dark:bg-box-dark-up dark:border-box-dark-up text-body dark:text-subtitle-dark" value="@isset($lesson->read_time){{$lesson->read_time}}@endisset" min="1" placeholder="Enter read time in minutes" required>
            </div>
            ...
            <div class="col-span-12 md:col-span-6">
                ...
                <div class="flex flex-wrap items-center gap-[15px]">
                    ...
                    <!--Second radio-->
                    <div class="mb-[0.125rem] mr-4 inline-block min-h-[1.5rem] pl-[1.5rem]">
                       <input class="relative ltr:float-left rtl:float-right -ms-[1.5rem] me-1 mt-0.5 h-[18px] w-[18px] appearance-none rounded-full border-1 border-solid border-normal before:pointer-events-none before:absolute before:h-4 before:w-4 before:scale-0 before:rounded-full before:bg-transparent before:opacity-0 before:shadow-[0px_0px_0px_13px_transparent] before:content-[''] after:absolute after:z-[1] after:block after:h-4 after:w-4 after:rounded-full after:content-[''] checked:border-primary checked:before:opacity-[0.16] checked:after:absolute checked:after:left-1/2 checked:after:top-1/2 checked:after:h-[0.625rem] checked:after:w-[0.625rem] checked:after:rounded-full checked:after:border-primary checked:after:bg-primary checked:after:content-[''] checked:after:[transform:translate(-50%,-50%)] hover:cursor-pointer hover:before:opacity-[0.04] hover:before:shadow-[0px_0px_0px_13px_rgba(0,0,0,0.6)] dark:border-neutral-600 dark:checked:border-primary dark:checked:after:border-primary dark:checked:after:bg-primary" type="radio" id="paid_no" name="paid" value="0" {{$lesson->is_free == 1 ? "checked" : ""}}>
            ...
            <div class="col-span-12 md:col-span-6">
               ...
               <div class="flex flex-wrap items-center gap-[15px]">
                   <!--First radio-->
                   <div class="mb-[0.125rem] mr-4 inline-block min-h-[1.5rem] pl-[1.5rem]">
                      <input class="relative ltr:float-left rtl:float-right -ms-[1.5rem] me-1 mt-0.5 h-[18px] w-[18px] appearance-none rounded-full border-1 border-solid border-normal before:pointer-events-none before:absolute before:h-4 before:w-4 before:scale-0 before:rounded-full before:bg-transparent before:opacity-0 before:shadow-[0px_0px_0px_13px_transparent] before:content-[''] after:absolute after:z-[1] after:block after:h-4 after:w-4 after:rounded-full after:content-[''] checked:border-primary checked:before:opacity-[0.16] checked:after:absolute checked:after:left-1/2 checked:after:top-1/2 checked:after:h-[0.625rem] checked:after:w-[0.625rem] checked:after:rounded-full checked:after:border-primary checked:after:bg-primary checked:after:content-[''] checked:after:[transform:translate(-50%,-50%)] hover:cursor-pointer hover:before:opacity-[0.04] hover:before:shadow-[0px_0px_0px_13px_rgba(0,0,0,0.6)] dark:border-neutral-600 dark:checked:border-primary dark:checked:after:border-primary dark:checked:after:bg-primary" type="radio" id="status_active" name="status" value="1" {{$lesson->status == 1 ? "checked" : ""}}>
